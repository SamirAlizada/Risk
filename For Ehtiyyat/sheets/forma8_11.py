...
# Forma8_11(1) istifadə edən məhsullar (3 il - 12 period)
FORMA8_11_TYPE1_PRODUCTS = [
    "(01)FerdiQeza",
    "(02)Tibbi",
...
# Forma8_11(2) istifadə edən məhsullar (5 il - 20 period)
FORMA8_11_TYPE2_PRODUCTS = [
    "(19)PesheMesuliyy",
...
def run_forma8_11(excel_file: str, ucot_file: str, reference_date: str):
    """Forma8_11 doldurur: Product-a görə düzgün sheet-i seçib Borc sheet-indən məlumat götürür"""
    ...
    use_type1 = product in FORMA8_11_TYPE1_PRODUCTS
    use_type2 = product in FORMA8_11_TYPE2_PRODUCTS
    
    if not use_type1 and not use_type2:
        print(f"  ⚠ {product}: Forma8_11 tip məlumatı tapılmadı, skip edilir")
        wb.save(excel_file)
        return
    
    if use_type1:
        print(f"    → Type1: Forma8_11(1) - 3 il (12 period)")
        sheet_to_keep = "Forma8_11(1)"
        sheet_to_delete = "Forma8_11(2)"
        num_periods = 12
        start_row = 24
        total_row = 25
    else:  # use_type2
        print(f"    → Type2: Forma8_11(2) - 5 il (20 period)")
        sheet_to_keep = "Forma8_11(2)"
        sheet_to_delete = "Forma8_11(1)"
        num_periods = 20
        start_row = 32
        total_row = 33
    ...
    ws = wb[sheet_to_keep]
    ws.title = "Forma8_11"
    print(f"    ✓ {sheet_to_keep} → Forma8_11 adına dəyişdirildi")
    ...
    if df_product.empty:
        ...
        df_product = pd.DataFrame(columns=["I", "II", "III", "IV"])
    ...
    if not df_product.empty:
        ...
        df_product["IV"] = pd.to_numeric(df_product["IV"], errors='coerce').fillna(0)
    ...
    # ================== D VƏ E SÜTUNLARI - PERIOD HESABLAMA ==================
    print(f"    {num_periods} period hesablanır (D{start_row} və E{start_row} → D13 və E13)...")
    ...
    for i in range(num_periods):
        ...
        if not df_product.empty:
            ...
            # D sütunu - III sütunundan
            period_sum_d = df_product.loc[mask, "III"].sum()
            # E sütunu - IV sütunundan
            period_sum_e = df_product.loc[mask, "IV"].sum()
        else:
            period_sum_d = 0
            period_sum_e = 0
    ...
    # E sütunu toplam
    ws.cell(row=total_row, column=5).value = round(grand_total_e, 2)
    ws.cell(row=total_row, column=5).font = bold_font
    ws.cell(row=total_row, column=5).border = thin
    ws.cell(row=total_row, column=5).alignment = center
    
    print(f"    ✓ D sütunu toplam D{total_row} = {grand_total_d:.2f}")
    print(f"    ✓ E sütunu toplam E{total_row} = {grand_total_e:.2f}")
    
    # ================== G SÜTUNU HESABLAMA (D - E + F) ==================
    print(f"    G sütunu hesablanır (D - E + F)...")
    ...
    print(f"    ℹ F sütunu boş qalır")
    
    wb.save(excel_file)
    print(f"  ✅ {product}: Forma8_11 tamamlandı")