...
def run_forma8_8(excel_file: str, reference_date: str, ucot_file: str):
    ...
    if "Forma8_7" not in wb.sheetnames:
        ...
        for row in range(source_start, source_end + 1):
            ...
            # Forma8_8-də target sütununa yaz
    ...
    # ================== S VƏ AA SÜTUNLARI (AVERAGE VƏ YA FAİZ8) ==================
    ...
    if has_zero:  # Nisbət sətirində 0 vardısa, Faiz sheet istifadə olunub
        print(f"    Faiz sheet-dən Faiz8 sütunu oxunur...")
        ...
        try:
            ...
            if not faiz_filtered.empty:
                ...
                # Faiz8 sütununu tap
                if 'Faiz8' in faiz_row.index:
                    faiz8_value = faiz_row['Faiz8']
                    
                    # Rəqəmə çevir
                    if not pd.isna(faiz8_value) and faiz8_value != '':
                        try:
                            faiz8_numeric = float(faiz8_value)
                            faiz8_numeric = round(faiz8_numeric, 4)
                            
                            if use_type1:
                                print(f"      → Type1: Faiz8 = {faiz8_numeric:.4f} → S13:S24")
                                
                                # S13:S24-ə Faiz8 dəyərini yaz
                                for row in range(13, 25):
                                    s_cell = ws.cell(row=row, column=19)  # S sütunu
                                    s_cell.value = faiz8_numeric
                                    s_cell.font = font_style
                                    s_cell.border = thin
                                    s_cell.alignment = center
                                    s_cell.number_format = '#,##0.0000'
                                
                                print(f"      ✓ Type1: S13:S24 = {faiz8_numeric:.4f} (Faiz8-dən)")
                            
                            else:  # Type2
                                print(f"      → Type2: Faiz8 = {faiz8_numeric:.4f} → AA13:AA32")
                                
                                # AA13:AA32-yə Faiz8 dəyərini yaz
                                for row in range(13, 33):
                                    aa_cell = ws.cell(row=row, column=27)  # AA sütunu
                                    aa_cell.value = faiz8_numeric
                                    aa_cell.font = font_style
                                    aa_cell.border = thin
                                    aa_cell.alignment = center
                                    aa_cell.number_format = '#,##0.0000'
                                
                                print(f"      ✓ Type2: AA13:AA32 = {faiz8_numeric:.4f} (Faiz8-dən)")
                        
                        except (ValueError, TypeError):
                            print(f"      ⚠ Faiz8 rəqəmə çevrilə bilmədi, ortalama hesablanacaq")
                            # Faiz8 yoxdursa və ya səhvdirsə, ortalama hesabla
                            has_zero = False  # Ortalamanı trigger et
                    else:
                        print(f"      ⚠ Faiz8 boşdur, ortalama hesablanacaq")
                        has_zero = False
                else:
                    print(f"      ⚠ Faiz8 sütunu tapılmadı, ortalama hesablanacaq")
                    has_zero = False
            else:
    ...
    # Əgər Faiz sheet istifadə olunmayıbsa və ya Faiz8 tapılmayıbsa, ortalama hesabla
    if not has_zero:
        if use_type1:
