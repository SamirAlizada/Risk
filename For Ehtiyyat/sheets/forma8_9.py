...
# Forma8_9 Type1 məhsullar (8_8-dən W31)
FORMA8_9_TYPE1_PRODUCTS = [
    "(01)FerdiQeza",
    "(02)Tibbi",
...
# Forma8_9 Type2 məhsullar (8_8-dən AE39)
FORMA8_9_TYPE2_PRODUCTS = [
    "(19)PesheMesuliyy",
...
def run_forma8_9(excel_file: str, reference_date: str, ucot_file: str):
    """Forma8_9 hazırlayır: Bütün product-lar üçün eyni sheet, sadəcə product adı dəyişir"""
    ...
    use_type1 = product in FORMA8_9_TYPE1_PRODUCTS
    use_type2 = product in FORMA8_9_TYPE2_PRODUCTS
    
    if not use_type1 and not use_type2:
        print(f"  ⚠ {product}: Forma8_9 tip məlumatı tapılmadı, skip edilir")
        wb.save(excel_file)
        return
    
    if use_type1:
        print(f"    → Type1: 8_8-dən W31 → E13")
        source_row = 31
        source_col = 23  # W sütunu
    else:  # use_type2
        print(f"    → Type2: 8_8-dən AE39 → E13")
        source_row = 39
        source_col = 31  # AE sütunu
    
    # ================== FORMA8_9 SHEET YOXLA ==================
    if "Forma8_9" not in wb.sheetnames:
        print(f"  ⚠ Forma8_9 sheet-i tapılmadı!")
        wb.save(excel_file)
        return
    
    ws = wb["Forma8_9"]
    print(f"    ✓ Forma8_9 sheet-i tapıldı")
    ...
    # ================== FORMA8_8-DƏN DATA KOPYALAMA ==================
    print(f"    Forma8_8-dən data kopyalanır...")
    
    if "Forma8_8" not in wb.sheetnames:
        print(f"    ⚠ Forma8_8 sheet-i tapılmadı, E13 boş qalacaq")
    else:
        ws_8_8 = wb["Forma8_8"]
        
        # Source cell-dən oxu
        source_cell = ws_8_8.cell(row=source_row, column=source_col)
    ...
    # ================== FORMA8_3-DƏN DATA KOPYALAMA (25%) ==================
    print(f"    Forma8_3-dən data kopyalanır (25%)...")

    if "Forma8_3" not in wb.sheetnames:
        print(f"    ⚠ Forma8_3 sheet-i tapılmadı, E14 boş qalacaq")
    else:
        ws_8_3 = wb["Forma8_3"]
        ...
        source_8_3_cell = ws_8_3.cell(row=source_8_3_row, column=7)
        source_8_3_value = source_8_3_cell.value or 0
        
        # 25% hesabla
        try:
            source_8_3_value = float(source_8_3_value)
            result_value = source_8_3_value * 0.25
            result_value = round(result_value, 2)
        except (ValueError, TypeError):
            result_value = 0.00
        
        # Target cell-ə yaz (E14)
        target_8_3_cell = ws.cell(row=14, column=5)  # E14
        target_8_3_cell.value = result_value
        target_8_3_cell.font = font_style
        target_8_3_cell.border = thin
        target_8_3_cell.alignment = center
        target_8_3_cell.number_format = '#,##0.00'
        
        if use_type1:
            print(f"      ✓ Type1: G25 ({source_8_3_value:.2f}) × 25% = {result_value:.2f} → E14")
        else:
            print(f"      ✓ Type2: G33 ({source_8_3_value:.2f}) × 25% = {result_value:.2f} → E14")

    # ================== FORMA8_7-DƏN DATA KOPYALAMA (2.5%) ==================
    print(f"    Forma8_7-dən data kopyalanır (2.5%)...")

    if "Forma8_7" not in wb.sheetnames:
        print(f"    ⚠ Forma8_7 sheet-i tapılmadı, E15 boş qalacaq")
    else:
        ws_8_7 = wb["Forma8_7"]
        ...
        for row in range(sum_start_row, sum_end_row + 1):
            cell_value = ws_8_7.cell(row=row, column=7).value or 0  # G sütunu (column=7)
            try:
        ...
        target_8_7_cell = ws.cell(row=15, column=5)  # E15
        target_8_7_cell.value = result_value
        target_8_7_cell.font = font_style
        target_8_7_cell.border = thin
        target_8_7_cell.alignment = center
        target_8_7_cell.number_format = '#,##0.00'
    ...
    wb.save(excel_file)
    print(f"  ✅ {product}: Forma8_9 tamamlandı")